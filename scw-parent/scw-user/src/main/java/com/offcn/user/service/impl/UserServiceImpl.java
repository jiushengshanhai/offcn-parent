...
public class UserServiceImpl implements UserService {
    ...
    public TMember login(String loginacct, String password) {
        ...
        TMemberExample.Criteria critera = example.createCriteria();
        critera.andLoginacctEqualTo(loginacct);
        List<TMember> tMembers = memberMapper.selectByExample(example);
        // password 是文明密码 ，要和数据库中加密之后的密码进行匹配
        BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
        if(tMembers != null && tMembers.size() == 1){
            TMember tMember = tMembers.get(0); // 从数据库中查询出来的数据
            // 用加密对象来匹配密码 （注意：千万不要使用密码字符串equals进行解密）
            boolean mathches = encoder.matches(password, tMember.getUserpswd());
            return mathches ? tMember : null;
